...
class picToanime:
    ...
    def savePicture(self,pic):
        ...
        # 将文件保存到地址 voice中
        picpath = '{}/{}'.format('AnimeGAN', picname)
        # 下载文件
    ...
    def convertPicture(self,picpath):
        ...
        if(self.style=='1'):
            ...
            model = hub.Module(name='animegan_v2_hayao_64', use_gpu=False)
        if not os.path.exists('AnimeGAN/Result'):
            os.mkdir('AnimeGAN/Result')
        shutil.rmtree('AnimeGAN/Result')
        result = model.style_transfer(images=[cv2.imread(picpath)], visualization=True,
                                      output_dir='AnimeGAN/Result')

    def run(self):
        picpath = self.savePicture(self.pic)
        self.convertPicture(picpath)
        filename = os.listdir("AnimeGAN/Result")[0]
        photo = '{}/{}/{}'.format('AnimeGAN','Result',filename)
        return photo
